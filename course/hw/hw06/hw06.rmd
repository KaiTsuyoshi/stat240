...
ggplot(lead_num, aes(x = num, y = n, fill = num)) +
  geom_col() +
  xlab("Leading Number") +
  ylab("Count") +
  ggtitle("Frequency of Leading Digits of Stars")
```
...
```{r}
non_conv = planets %>% 
  mutate(match = str_c("^", star," ", "[:lower:]$")) %>% 
  mutate(match = str_replace(match, "\\+", "\\\\+")) %>% 
  filter(!str_detect(planet, match))
...
```{r}
bat_split = batters %>% 
  transmute(name = str_extract(Name, "^[^\\*,^#,^\\\\]+"), 
            ID = str_extract(Name, "(?<=\\\\).*$"), 
            Bats = case_when(
    str_detect(Name, "\\*\\\\") ~ "Left",
    str_detect(Name, "\\#\\\\") ~ "Switch",
    str_detect(Name, "\\\\") ~ "Right")) 
  

print(bat_split, n = 20)

pitch_split = pitchers %>% 
  transmute(name = str_extract(Name, "^[^\\*,^#,^\\\\]+"), 
            ID = str_extract(Name, "(?<=\\\\).*$"), 
            Throws = case_when(
    str_detect(Name, "\\*\\\\") ~ "Left",
...
chars = batters %>% 
  select(`Pos Summary`) %>% 
  summarize(text = str_c(`Pos Summary`, collapse = ""))

char = tibble(
  character = str_split(chars, "") %>% 
    unlist()) %>% 
  arrange(character) %>% 
...
```{r}
mult = batters %>% 
  select(Name, Tm, Lg, `Pos Summary`) %>% 
  group_by(Name) %>% 
  summarize(Name, Tm, Lg, `Pos Summary`, n = n()) %>% 
  filter(n > 1, Tm == "TOT") 

  
sing = batters %>%
  select(Name, Tm, `Pos Summary`) %>%
  group_by(Name) %>%
  summarize(Name, Tm, `Pos Summary`, n = n()) %>%
  filter(n == 1)
...
joined = full_join(mult, sing) %>%
  ungroup() %>%
  left_join(pitch) 

primary = joined %>%
  mutate(name = str_extract(Name, "^[^\\*,^#,^\\\\]+"), ID = str_extract(Name, "(?<=\\\\).*$")) %>%
  mutate(pos = str_replace(`Pos Summary`, "^[\\*,\\/]", "")) %>%
  mutate(Primary_Position = str_extract(pos, "^.")) %>%
...
bats = batters %>% 
  mutate(name = str_extract(Name, "^[^\\*,^#,^\\\\]+"), ID = str_extract(Name, "(?<=\\\\).*$")) %>%
  group_by(name) %>% 
  summarize(name, ID, PA, HR, Lg, Tm, `Pos Summary`, n = n()) %>% 
  filter(n > 1, Tm == "TOT", Lg == "MLB") %>% 
  mutate(HR_rate = 100 * HR / PA) %>% 
  select(name, ID, PA, HR, HR_rate)

final = left_join(primary, bats, by = "name") %>% 
  filter(PA > 50) 
final %>% 
  mutate(Primary_Position = reorder(Primary_Position, HR_rate)) %>% 
