...
Each row is a country as a list of its characteristics (name, latitude, currency, etc.) as elements

What does the value of `n` in row 10 of the tibble represent?

Represents the number of elements (characteristics in this case) in the list of country #10 
...
No, as each element is a basic characteristic of every country and things such as countries sharing borders that could differ in number from country to country are stored in nested lists or arrays, thus not affecting original elements.
...
map() returns list, map_chr(), _dbl(), _int(), _lgl() return vectors of specific types (character, double, integer, logical)
...
```{r}
mult_cap = countries_df %>% 
  filter(countries_df$num_capitals > 1)
...
```{r}
na_country = countries_df %>% 
  mutate(num_capitals = case_when(is.na(capitals) ~ 0L,
                                  num_capitals == 0 ~ 0L,
                                  str_length(capitals) == 0 ~ 0L,
                                  str_length(capitals) > 0 ~ num_capitals),
         capitals = case_when(str_length(capitals) == 0 ~ NA_character_,
                              str_length(capitals) > 0 ~ capitals))
...
```{r}
un = countries_df %>% 
  filter(countries_df$un_member == TRUE) %>% 
