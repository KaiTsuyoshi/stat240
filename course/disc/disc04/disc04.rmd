...
```{r}
### Load a file where the delimiter character is a (a) tab
atl = read_tsv("../../data/atlanta-2018.tsv") %>%
  ## (b) select all variables except those that end with "wait"
  select(-ends_with("wait")) %>%
  ## (c) select all variables that does not contain "_n_"
  select(-contains("_n_")) %>%
  ...
  summarize(passengers = sum(passengers),
            flights = sum(flights)) %>%
  ## (d) Creates a “date” variable that makes all the dates in the date column into “year- month- day” format
...
atl_sum = atl %>% 
  group_by(wday) %>% 
  summarize(avg_flights = mean(flights), avg_pass = mean(passengers)) %>% 
  arrange(wday)
...
```{r}
chc = read_csv2("../../data/chicago-2018.csv") %>% 
  select(-ends_with("wait")) %>%
  select(-contains("_n_")) %>%
  select(-terminal,-hour,-all_excluded,-all_booths) %>%
  rename(passengers = all_total,
...
ggplot(chc_mean, aes(x = wday, y = avg_pass))+
  geom_col() +
  xlab("Day of Week") +
  ylab("Count")
